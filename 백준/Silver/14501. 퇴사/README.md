...
### 풀이
우선 7일간의 시간을 벗어나지 않아야한다. 즉 n일에 T[N]이 주어진 시간보다 작거나 같아야 한다 =>  i + T[i] <= case

1일에 3이 걸리는 시간은 10을 벌 수 있다. 그렇다는 것은 3일에 dp가 10이됨. dp[i + T[i]] = 10, 이고 dp[1]엔 cost가 없음. 2일까지  [0, 0, 0, 10, 0, 0, 0, 0]
2일에 5가 걸리는 시간은 20을 벌 수 있다. i+T[i] = 1 + 5 <= 6 이므로 dp[6]엔 우선 20이 들어간다. [0, 0, 0, 10, 0, 0, 20, 0]
3일차엔 1일차의 일과 마찬가지로 10이므로 dp는 같음 [0, 0, 0, 10, 0, 0, 20, 0]
4일차는 1일의 시간을 소모한다. 4일의 비용 + 3일까지의 비용(dp[3]), dp[4] 에서 비교를 한다.      [0, 0, 0, 10, 30, 0, 20, 0]
5일차는 2일의 시간을 소모함. [0, 0, 0, 10, 30, 30, 45, 0] 
6일차와 7일차는 일을 하지못함 [0, 0, 0, 10, 30, 30, 45, 0] , [0, 0, 0, 10, 30, 30, 45, 45]
